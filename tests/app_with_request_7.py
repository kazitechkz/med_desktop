...
def save_screenshot():
    ...
    with open(file_path, 'rb') as f:
        files = {'file': f}
        response = requests.post('http://localhost:5000/upload', files=files, data=[])  # URL сервера
...
info_label = tk.Label(left_frame, text="Точность измерения", font=("Arial", 14), justify=tk.LEFT)
info_label.pack()
